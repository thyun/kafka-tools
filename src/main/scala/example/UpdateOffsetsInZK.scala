...
object UpdateOffsetsInZK {
  ...
  def main(args: Array[String]) {
    if(args.length < 3)
      usage
    var offsetOption = OffsetRequest.EarliestTime
    args(0) match {
      case Earliest => offsetOption = OffsetRequest.EarliestTime
      case Latest => offsetOption = OffsetRequest.LatestTime
      case _ => usage
    } 
    ...
    val topic = args(2)
    if(args.length == 3) 
      getAndSetOffsets(zkClient, offsetOption, config, topic)
    else {
      val partition = args(3).toInt
      getAndSetOffset(zkClient, offsetOption, config, topic, partition)
    }
  }
  ...
  private def getAndSetOffset(zkClient: ZkClient, offsetOption: Long, config: ConsumerConfig, topic: String, partition: Int): Unit = {
  ...
  private def usage() = {
    println("USAGE: " + UpdateOffsetsInZK.getClass.getName + " [earliest | latest] consumer.properties topic [partition]")
    System.exit(1)
  }
}
